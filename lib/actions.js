 ...
 * @actions
 */
let TouchAction = function (driver) {
  this.driver = driver;
  this.gestures = [];
};
 ...
 * @actions
 */
let MultiAction = function (browserOrElement) {
...
MultiAction.prototype.add = function () {
  let actions = __slice.call(arguments, 0);
...
InputDevice.prototype.pause = function (opts) {
  let duration = _.isNumber(opts) ? opts : opts.duration;
  this.addAction("pause", {duration});
};
...
let W3CActions = function (driver) {
  this.driver = driver;
  this.touchCount = 1;
  this.inputs = {};
};
...
W3CActions.prototype.addInputDevice = function (inputSource) {
  if (_.isNil(inputSource.id)) {
    if (inputSource.type === "pointer") {
      let pointerType = inputSource.parameters.pointerType;
...
W3CActions.prototype.toJSON = function () {
  let actions = _.values(this.inputs, function (input) { input.toJSON(); });
  return {
    actions
  };
};

...
let Webdriver = module.exports = function (configUrl) {
...
Webdriver.prototype._init = function () {
  ...
  // http options
  let httpOpts = httpUtils.newHttpOpts('POST', _this._httpConfig);

  let url = httpUtils.buildInitUrl(this.configUrl);

  // building request
  let data = {desiredCapabilities: _desired};
  ...
  httpUtils.requestWithRetry(httpOpts, this._httpConfig, this.emit.bind(this), function (err, res, data) {
    ...
    if (!_this.sessionID) {
      ...
      try {
        let locationArr = res.headers.location.replace(/\/$/, '').split('/');
...
Webdriver.prototype._jsonWireCall = function (opts) {
  ...
  // http options init
  let httpOpts = httpUtils.newHttpOpts(opts.method, this._httpConfig);

  let url = httpUtils.buildJsonCallUrl(this.noAuthConfigUrl, this.sessionID, opts.relPath, opts.absPath);
...
Webdriver.prototype._sauceJobUpdate = function (jsonData, done) {
  ...
  let jobUpdateUrl = url.resolve(
    this.sauceRestRoot.replace(/\/?$/, '/'),
    this.configUrl.auth.replace(/:.*$/, '') + '/jobs/' + this.sessionID);

  let httpOpts = httpUtils.newHttpOpts('PUT', this._httpConfig);
  httpOpts.auth = {

...
commands.waitFor = function () {
  ...
  let _this = this;
  let endTime = Date.now() + opts.timeout;

  let unpromisedAsserter = new Asserter(
    function (browser, cb) {
      let promise = opts.asserter.assert(browser, cb);
      if (promise && promise.then && typeof promise.then === 'function') {
        promise.then(
          function (res) { cb(null, true, res); },
          function (err) {
            if (err.retriable) { cb(null, false); } else { throw err; }
          }
        );
      }
    }
...
commands.waitForElement = function () {
  ...
  let unpromisedAsserter = new Asserter(
    function (el, cb) {
      let promise = opts.asserter.assert(el, cb);
      if (promise && promise.then && typeof promise.then === 'function') {
        promise.then(
  ...
  let wrappedAsserter = new Asserter(
    function (browser, cb) {
...
let addMethodsForSuffix = function (type, singular, plural) {
  if (singular) {
    /**
...
commands.performMultiAction = function () {
  ...
  try {
    let data = multiTouchAction.toJSON(element);
...
// deprecated
let _isVisible1 = function (element) {
  let cb = findCallback(arguments);
...
// deprecated
let _isVisible2 = function (queryType, querySelector) {
  let cb = findCallback(arguments);
...
commands.waitForJsCondition = function () {
  ...
  let cb = findCallback(arguments);
  let fargs = utils.varargs(arguments);
  let jsConditionExpr = fargs.all[0],
      timeout = fargs.all[1],
      pollFreq = fargs.all[2];
  commands.waitFor.apply(this, [
    {
...
commands.elementByImageFile = function () {
  ...
  fs.readFile(imgPath, function (err, fileData) {
    if (err) {return cb(err);}
    let elFn = multiple ? 'elementsByImage' : 'elementByImage';

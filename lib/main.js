...
function buildConfigUrl (remoteWdConfig) {
  let configUrl = _(remoteWdConfig).clone();
  ...
  let parsed = url.parse(url.format(configUrl), true);
...
let parseRemoteWdConfig = function (args) {
  let config;
...
function remote () {
  ...
  let driverTypes = _(driverProtos).keys().value();
  let driverType = driverTypes.indexOf(_(args).last()) > 0 ? args.pop() : 'async';

  let rwc = parseRemoteWdConfig(args);
  return new (driverProtos[driverType])(rwc);
}

function wrap () {
  let promiseProtos = require('./promise-webdriver')(Webdriver, Element, false);
  PromiseWebdriver = promiseProtos.PromiseWebdriver;
  PromiseElement = promiseProtos.PromiseElement;
  let promiseChainProtos = require('./promise-webdriver')(Webdriver, Element, true);
...
function addPromiseChainMethod (name, method) {
  let wrappedMethod = function () {
    let args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    let promise = new Q(method.apply(this, args));

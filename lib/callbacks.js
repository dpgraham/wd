...
let callbackWithDataBase = function (cb) {
  cb = cb || cbStub;
  return function (err, data) {
    if (err) { return cb(err); }
    let obj,
    ...
    if (obj.status > 0) {
      let jsonwireError = getJsonwireError(obj.status);
...
exports.elementCallback = function (cb, browser) {
  ...
  return callbackWithDataBase(function (err, obj) {
    ...
    if (!obj.value.ELEMENT) {
      ...
      let el = browser.newElement(obj.value.ELEMENT);
      cb(null, el);
    }
  });
};
...
exports.elementsCallback = function (cb, browser) {
  ...
  return callbackWithDataBase(function (err, obj) {
    ...
    for (i = 0; i < obj.value.length; i++) {
      let el = browser.newElement(obj.value[i].ELEMENT);
      elements.push(el);
    }
    cb(null, elements);
  });

...
let elementChainableMethods = ['clear', 'click', 'doubleClick', 'doubleclick',
  ...
  PromiseWebdriver.prototype._enrich = function (obj, currentEl) {
    ...
    if (utils.isPromise(obj) && !obj.__wd_promise_enriched) {
      ...
      _(promise).functionsIn()
        ...
        .each(function (fname) {
          let _orig = promise[fname];
          promise[fname] = function () {
            let subobj = _orig.apply(this, __slice.call(arguments, 0));
      ...
      let promisedMethods = filterPromisedMethods(Object.getPrototypeOf(_this));
      _this.sampleElement = _this.sampleElement || _this.newElement(1);
      let elementPromisedMethods = filterPromisedMethods(Object.getPrototypeOf(_this.sampleElement));
      let allPromisedMethods = _.union(promisedMethods, elementPromisedMethods);
